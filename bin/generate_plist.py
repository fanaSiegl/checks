...
def loadChecks():
	...
	for modulePath in glob.glob(os.path.join(util.getPathChecks(), util.CHECK_PATTERN)):
	...
	is_saved = base.CheckDescription.save(checkDescriptions, os.path.join(
		util.getPlistPath(), util.PLIST_NAME))
...
'''
	...
	if DEBUG:
		...
		return
	fo = open(
		os.path.join(util.PATH_DOC, 'sphinx', 'source', util.CHECKS_DOC_RST), 'wt')
	fo.write(content)
	fo.close()
...
def saveModuleDoc(moduleName, docString):
	...
	if DEBUG:
		...
		return
	fo = open(
		os.path.join(util.PATH_DOC, 'sphinx', 'source', '%s.rst' % moduleName), 'wt')
	fo.write(docString)
	fo.close()
